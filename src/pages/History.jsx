...
const History = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await axios.get(`${serrverUrl}/api/history`, {
  ...
  return (
    <>
      <div className="min-h-screen bg-[#F7F4ED] px-10 py-16 relative">
        <button
          onClick={() => navigate(-1)}
          className="absolute top-8 right-10 px-5 py-2 rounded-full bg-[#473628] text-white font-semibold hover:bg-[#5a4335] transition-all shadow-md"
        >
          ← Back
        </button>

        <div className="mb-16">
          <h1 className="text-5xl font-black text-[#473628] tracking-wide border-b-4 border-b-current/10 inline-block pb-1">
            Recently Viewed 📚
          </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 xl:grid-cols-7 gap-y-16 gap-x-8 justify-items-center">
          {history.map((item, index) => (
            ...
            >
              <div className="w-full h-[225px] rounded-xl overflow-hidden relative transition-all duration-300 ease-out transform shadow-xl border-2 border-transparent hover:border-4 hover:border-yellow-400/50 group-hover:scale-[1.07] group-hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(0,0,0,0.35)]">
                <img
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-end justify-start p-3">
              ...
              <p className="text-[17px] font-bold text-[#3A2B21] text-center mt-4 line-clamp-2 group-hover:text-[#A67344] transition-colors">
                {item.title}
              </p>
